...
/* 
   ...
   This module provides an emulation of the memory system for the MOS Technology 6502 processor.

   The MOS 6502 processor, utilized in various computers and game consoles of the 1970s and 1980s, interacts with
   an addressable memory space. This module emulates a simplified version of this memory system, allowing the
   6502 instructions to read from and write to memory.

   The emulated memory is represented by a structure called MEM6502, containing an array of bytes to mimic RAM.
   The maximum size of this memory is defined as MAX_MEM (65 Kilobytes).

   This file implement instructions for functions defined in mem6502.h

   Functions:
   - initializeMem6502: Initializes the emulated memory, allocating space for 65 Kilobytes of RAM.
   - freeMem6502: Frees the allocated memory used for emulating the 6502 memory system.
   - ReadByte: Reads a byte of data from emulated memory at the specified address.
...
void freeMem6502(MEM6502 *memory) {
    free(memory->Data);
    memory->Data = NULL;  // Optional: define data as null after freeing the memory.
}
