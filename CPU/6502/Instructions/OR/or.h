...
/* 
   ...
   OR works by moving a value into the Accumulator register (A).
   For more information about the instructions, refer to Instructions.MD
*/

/*
   OR (Load Accumulator) instruction supports various addressing modes in the 6502 architecture.
...
/*
   OR_IM - Load Accumulator with Immediate value.
...
void OR_IM(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
...
/*
   OR_ZP - Load Accumulator from Zero Page.
...
void OR_ZP(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
...
/*
   OR_ZPX - Load Accumulator from Zero Page with X Offset.
...
void OR_ZPX(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
    Byte ZeroPageAddr = FetchByte(Cycles, memory, cpu);
    ZeroPageAddr += cpu->X;
    (*Cycles)--;
    cpu->A = ReadByte(Cycles, ZeroPageAddr, memory) | cpu->A;
...
/*
   OR_ABS - Load Accumulator from Absolute address.
...
void OR_ABS(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
...
/*
   OR_ABSX - Load Accumulator from Absolute address with X Offset.
   Similar to OR_ABS, but adds the X register value to the absolute address before reading
   the value from memory. It adjusts the cycle count accordingly OR sets the status flags.
*/


void OR_ABSX(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
    Word Absolute = FetchWord(Cycles, memory, cpu);
    Absolute += cpu->X;
    cpu->A = ReadByte(Cycles, Absolute, memory) | cpu->A;
    ORSetStatus(cpu);
...
/*
   OR_ABSY - Load Accumulator from Absolute address with Y Offset.
   Similar to OR_ABS, but adds the Y register value to the absolute address before reading
   the value from memory. It adjusts the cycle count accordingly OR sets the status flags.
*/


void OR_ABSY(Word *Cycles, MEM6502 *memory, CPU6502 *cpu) {
    Word Absolute = FetchWord(Cycles, memory, cpu);
    Absolute += cpu->Y;
    cpu->A = ReadByte(Cycles, Absolute, memory) | cpu->A;
    ORSetStatus(cpu);
